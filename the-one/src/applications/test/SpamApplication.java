...
public class SpamApplication extends Application {
	 ...
	 */
	public SpamApplication(Settings s) {
	 ...
	 * @param a
	 */
	public SpamApplication(SpamApplication a) {
		super(a);
	...
	public Application replicate() {
		return new SpamApplication(this);
	}
	...
	public void update(DTNHost host) {
		...
		if (curTime - this.lastPing >= this.interval) {
			...
			for (int i = 0; i < SimScenario.getInstance().getHosts().size(); i++) {
				if (i == host.getAddress()) continue;

				Message m = new Message(host, SimScenario.getInstance().getWorld().getNodeByAddress(i), "ping" +
						SimClock.getIntTime() + "-" + host.getAddress()+ "->" + i,
						getPingSize());
